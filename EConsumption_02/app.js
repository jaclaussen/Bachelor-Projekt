...
.directive('econsumption02', ['logger', 'bngApi', 'StreamsManager', 'UiUnits', function (logger, bngApi, StreamsManager, UiUnits) {
  return {
    template: 
        '<object id="cool_thing_200019" style="width:100%; height:100%; box-sizing:border-box; pointer-events: none" type="image/svg+xml" data="modules/apps/EConsumption_02/econsumption.svg?t=' + Date.now() + '"/>',
    replace: true,
    restrict: 'EA',
        ...
        scope.$on('streamsUpdate', (ev, streams) => {
          ...
          let speed = streams.electrics.wheelspeed;
			...
			if (streams.electrics.currentSeqNumber ==-1){
				document.getElementById('cool_thing_200019').style.display = "none";
			} else if ((streams.electrics.currentCounter==5 && streams.electrics.aFirst) || (streams.electrics.currentCounter==4 && !streams.electrics.aFirst)) {
				document.getElementById('cool_thing_200019').style.display = "block";
			} else {
				document.getElementById('cool_thing_200019').style.display = "none";
			}
